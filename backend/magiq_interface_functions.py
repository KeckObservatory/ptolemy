...
def convert_target_to_targetlist_row(target, acquisition, idx):
    ...
    if aparams:
        ...
        rowStr += 'raOffset=' + str(raOffset) + ' ' if isinstance(raOffset, bool)
        rowStr += 'decOffset=' + str(decOffset) + ' ' if isinstance(decOffset, bool)
        rowStr += 'rotmode=' + str(rotMode) + ' ' if isinstance(rotMode, bool)
        rowStr += 'wrap=' + str(wrap) + ' ' if isinstance(wrap, bool)
    return rowStr
