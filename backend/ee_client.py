...
def get_ee_state():
    ...
    try:
        ...
        allSeq = [ *ee.seq_q.sequences, *ee.seq_q.boneyard ] 
        data['selected_task'] = next((x for x in allSeq \
        ...
        isPaused = 'true' in ktl.read(config_parser['KTL']['service'], 'pause')
        isHalted = 'false' in ktl.read(config_parser['KTL']['service'], 'halt')
        data['pause'] = isPaused
        data['halt'] = isHalted
...
def ee_refresh_queues():
    try:
        ...
        ee.seq_q.sequences = freshSequenceList
